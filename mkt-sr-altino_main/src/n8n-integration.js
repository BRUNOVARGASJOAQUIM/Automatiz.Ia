...
class N8NIntegration {
  ...
  novoVIP(nome, whatsapp, email, aniversario = null, onSuccess = null, onError = null) {
    ...
    return this.enviarWebhook('novo_vip', {
      ...
      aniversario, data_cadastro: new Date().toISOString()
    }, onSuccess, onError);
  }

  novaReserva(nome, whatsapp, email, tipoEvento, pessoas, dataEvento, observacoes = '', onSuccess = null, onError = null) {
    const tel = this._formatarTel(whatsapp);
    if (!tel) { if (onError) onError(new Error('WhatsApp inválido')); return; }
    return this.enviarWebhook('nova_reserva', {
      nome: nome.trim(), whatsapp: tel, email: (email || '').trim(),
      tipo_evento: tipoEvento, pessoas: parseInt(pessoas),
      data_evento: dataEvento, observacoes: (observacoes || '').trim(),
